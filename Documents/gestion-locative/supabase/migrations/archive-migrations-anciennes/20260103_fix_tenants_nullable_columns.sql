-- ============================================================================
-- FIX: Colonnes NULLABLE dans table tenants
-- Date: 2026-01-03
...
1. **Sécurité RLS**: Les policies RLS vérifient user_owns_entity(entity_id).
   Si entity_id est NULL, la policy ne peut pas fonctionner correctement.

2. **Isolation multi-tenant**: Un tenant sans entity_id pourrait être
   visible par tous les utilisateurs (fuite de données).

3. **Conformité RGPD**: Chaque tenant DOIT appartenir à une entité
   juridique identifiée.

⚠️  CONSÉQUENCES:
- Si vous aviez des tenants avec entity_id = NULL, ils seront SUPPRIMÉS
- Cela garantit qu'aucun tenant orphelin ne peut exister
- Les futures insertions DOIVENT fournir un entity_id valide

✅ APRÈS CETTE MIGRATION:
- Impossible de créer un tenant sans entity_id
- Impossible de créer un tenant sans user_id
- La sécurité RLS fonctionne à 100%
*/
